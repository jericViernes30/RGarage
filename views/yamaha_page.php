...
<body class="w-full bg-gray-200 min-h-screen overflow-y-auto text-black-v1">
    ...
    <div class="w-full relative mb-10 mx-auto block">
        <img src="/RGarage/public/images/v2-YAMAHA.jpg" alt="" class="h-[150px] filter-bluish w-full">
        <div class="absolute top-10 left-40">
            <p class="text-black text-3xl font-bold">MOTORCYCLES</p>
            <a class="text-black" href="/RGarage/">Home</a> <span class="text-black">>></span> <a class="text-black" href="/Rgarage/user/unitsAvailable">Motorcycles</a><span class="text-black">>></span> <span class="text-black text-sm font-medium uppercase">yamaha</span>
        </div>
    </div>
    ...
    $('#livesearch').off('keyup').on('keyup', function() {
        ...
        searchTimeout = setTimeout(function() {
            $.ajax({
                ...
                url: '/RGarage/user/livesearch-yamaha',
                data: { key: key },
                dataType: 'json',
    ...
    $.ajax({
        method: 'GET',
        url: '/RGarage/user/filter-yamaha', // Update with actual path to controller
