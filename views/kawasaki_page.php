...
<body class="w-full bg-gray-200 min-h-screen overflow-y-auto text-black-v1">
    ...
    <div class="w-full relative mb-10 mx-auto block">
        <img src="/RGarage/public/images/v2-KAWASAKI.jpg" alt="" class="h-[150px] filter-bluish w-full">
        <div class="absolute top-10 left-40">
            <p class="text-black text-3xl font-bold">MOTORCYCLES</p>
            <a class="text-black" href="/RGarage/">Home</a> <span class="text-black">>></span> <a class="text-black" href="/Rgarage/user/unitsAvailable">Motorcycles</a><span class="text-black">>></span> <span class="text-black text-sm font-medium uppercase">kawasaki</span>
        </div>
    </div>
    ...
    <div id="livesearch-results" class="w-[80%] flex items-center gap-4 flex-wrap overflow-hidden mx-auto">
        ...
        <div id="result" class="w-[24%] h-[20%] bg-white rounded-lg mb-6">
            <button onclick="window.location.href='/RGarage/user/unit-detail?unitID=<?php echo htmlspecialchars($unit['id']) ?>'" class="p-2 overflow-hidden">
                <img src="/RGarage/public/images/<?php echo $firstImage ?>" alt="<?php echo $firstImage ?>" class=" rounded-tl-lg rounded-tr-lg hover:scale-125 transition duration-175 ease-in-out">
            </button>
            <div class="">
    ...
    $('#livesearch').off('keyup').on('keyup', function() {
        ...
        searchTimeout = setTimeout(function() {
            $.ajax({
                ...
                url: '/RGarage/user/livesearch-kawasaki',
                data: { key: key },
                dataType: 'json',
    ...
    $.ajax({
        method: 'GET',
        url: '/RGarage/user/filter-kawasaki', // Update with actual path to controller
